function ax_space = animateDrift(Ux, Uy, r,x,y,yaw,slip_f, slip_r, delta_rad, veh, dT )
...
tire_color = [1 0 0];                   % color of tires
...
sim.traj                = plot(0,0, 'Color',[0,0,1,1],'LineWidth',2);
...
factor = 2.;
xlim(ax_space, [ min(posE_m)-factor*5, max(posE_m)+factor*5 ]);
ylim(ax_space, [ min(posN_m)-factor*4, max(posN_m)+factor*4 ]);
...
update_axes();
    ...
    function update_axes()
        ...
        % Plot the vehicle
        update_vehicle(psi_rad(curI), posE_m(curI), posN_m(curI), delta_mag_factor*delta_rad(curI), slip_f(curI), slip_r(curI));
    ...
    function update_vehicle(psi, posE, posN, delta, slip_f_curr, slip_r_curr)
        a = veh.a;
        b = veh.b;
        rW = .34;       % tire radius
