...
classdef ilqr < handle
    ...
    methods
        % Constructor
        function self = ilqr(init_state,target_states,initial_guess,dt,start_time,end_time,f_disc,A,B,Q_k,R_k,Q_T,parameters,n_iterations)
            ...
            self.inputs_ = self.target_inputs_;
            self.states_ = self.target_states_;
        ...
        function [states,inputs,k_feedforward,K_feedback,current_cost] = solve(self)
            ...
            [new_states,new_inputs]=forwards_pass(self,0);
        ...
        function total_cost = compute_cost(self,states,inputs)
            ...
            for ii = 1:self.n_timesteps_
                ...
                current_x_des = self.target_states_(ii,:)';
                current_u_des = prev_inputs(ii,:)';
                
                % Compute differences from the target 
                % ******changed cost for u to discourage big changes
                x_difference = current_x_des - current_x;
                u_difference = current_u - current_u_des;
                % u_difference = current_u_des - current_u;
                current_cost =  u_difference'*self.R_k_*u_difference... 
                                + x_difference'*self.Q_k_*x_difference; 
                total_cost = total_cost+current_cost;
            end
            % Compute terminal cost
             terminal_x_diff = self.target_states_(end,:)'-states(end,:)';
        ...
        function [k_trj,K_trj,expected_cost_redu] = backwards_pass(self)
            ...
            V_x = self.Q_T_*(self.states_(end,:)'-self.target_states_(end,:)');
            V_xx = self.Q_T_;
            
            prev_inputs = circshift(self.inputs_, 1, 1); % cost on changing of inputs
            prev_inputs(1,:) = prev_inputs(2,:); % [1,1,2,3,4]
