classdef quadprog_mpc < handle
    % This class sets up the optimization problem with the constructor and
    % solve_quadprog will return the optimal set of inputs and gains to track a
    % target trajectory
    ...
    methods
        ...
        function self = quadprog_mpc(target_states,initial_guess,dt,horizon,f_disc,A,B,Q_k,R_k,Q_T,constraints,n_iterations)
        ...
        function [states,inputs,current_cost] = solve_ilqr(self,current_idx,current_state)
            ...
            %%%need to figure out how to write the cost for quadprog
...
%                     disp("Stopping optimization, low learning rate");
        ...
        function [states,inputs] = rollout(self)
            ...
            for ii=1:self.n_timesteps_
                ...
                next_state = self.f_(current_state,current_input,self.dt_,self.constraints_);
...
%                 l_x = zeros(size(current_x)); % Defined as zero right now because there is no desired trajectory
                ...
                A_k = self.A_(current_x,current_u,self.dt_,self.constraints_);
                B_k =self.B_(current_x,current_u,self.dt_,self.constraints_);
