...
%% plotting
    ...
    function plot_body(A,R,z)
        ...
        check_body = check_if_plot_is_available(A,'body');
        % check_body = false;
        if check_body
            A.plot_data.body.Vertices = VB ;
            A.plot_data.sides.Vertices = VB ;
            A.plot_data.rotor.Vertices = VR ;
            % A.plot_data.body.EdgeColor = A.plot_body_edge_color;
        else
    ...
    function body_polytope = footprint_polytope(A)
        % SAFE, AGGRESSIVE QUADROTOR FLIGHT VIA REACHABILITY-BASED
        % TRAJECTORY DESIGN pg.4

        thickness = 0.54;
        width     = 0.54;
        height    = 0.0855;

        body_polytope = make_set_cuboid3D("center", zeros(3, 1), ...
