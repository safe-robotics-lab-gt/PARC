classdef turtlebot_iLQR_MPC_LLC < turtlebot_LLC
    ...
    methods
        %% constructor
        function LLC = turtlebot_iLQR_MPC_LLC(varargin)
        ...
        function [states,inputs,k_feedforward,K_feedback, cost_arr] = iterate(LLC, z_init, T, U, Z, n_iterations)
            ...
            % initial guess
            %[states,inputs] = LLC.rollout();
            % instead of rollout the initial backward forward pass was
            % better
        ...
        function [states,inputs] = rollout(LLC)
            ...
            current_state = LLC.current_state_;
        ...
        function [k_trj,K_trj,expected_cost_redu] = backwards_pass(LLC)
            ...
            % Initialize gradient and hessian of the value function
        ...
        function U = get_control_inputs(LLC,A,t_cur,z_cur,T_des,U_des,Z_des)
            ...
            if isempty(Z_des)
                ...
                %[u_opt,z_opt] = match_trajectories(t_cur,T_opt,U_opt,T_opt,Z_opt,LLC.interp_mode) ;
