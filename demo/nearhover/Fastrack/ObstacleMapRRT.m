classdef ObstacleMapRRT < handle
  ...
  methods
    ...
    function self = ObstacleMapRRT(obs)
      ...
      % (4=num_vertices_per_plane, 3=xyz, 6=num_of_faces) 
...
%           small = 1e-3;
    ...
    function pad(self, err, obs)
      ...
      if length(err) ~= size(obs, 2)
          err = repmat(err, [1, size(obs, 2)]);
      end
      one = unique(obs(:, 1));
      two = unique(obs(:, 2));
      if length(one) == 1
      ...
      if index == 2 % if obstacle is flat on y-plane
        new_obs_front{1} = [pointOne(1) - err(1), pointOne(2) + err(2), pointOne(3) - err(3)];
        new_obs_front{2} = [pointTwo(1) - err(1), pointTwo(2) + err(2), pointTwo(3) + err(3)];
        new_obs_front{3} = [pointThree(1) + err(1), pointThree(2) + err(2), pointThree(3) + err(3)];
        new_obs_front{4} = [pointFour(1) + err(1), pointFour(2) + err(2), pointFour(3) - err(3)];
        
        self.indexx = self.indexx + 1;
        self.padded_obs(:,:, self.indexx) = cell2mat(new_obs_front);
        
        new_obs_back{1} = [pointOne(1) - err(1), pointOne(2) - err(2), pointOne(3) - err(3)];
        new_obs_back{2} = [pointTwo(1) - err(1), pointTwo(2) - err(2), pointTwo(3) + err(3)];
        new_obs_back{3} = [pointThree(1) + err(1), pointThree(2) - err(2), pointThree(3) + err(3)];
        new_obs_back{4} = [pointFour(1) + err(1), pointFour(2) - err(2), pointFour(3) - err(3)];
        self.indexx = self.indexx + 1;
