function [trueQuad, virt_x, controller] = ...
  simulateFastrack  (sD_X, derivX, sD_Z, derivZ, TEB, obs, goal, varargin)
...
t_max = 10;
if isfield(kwargs, 't_max')
    t_max = kwargs.t_max;
end

switch_control_margin = 0.25;
if isfield(kwargs, 'switch_control_margin')
    switch_control_margin = kwargs.switch_control_margin;
end

control_gain = [20; 20; 0.1];
if isfield(kwargs, 'control_gain')
...
if isfield(kwargs, 'visualize')
  vis = kwargs.visualize;
else
  vis = true;
end

% start
if isfield(kwargs, 'start')
    start = kwargs.start;
else
    start = [-1; 0; 5];
end
...
% Write video
write_video = false;
if isfield(kwargs, 'write_video')
    write_video = kwargs.write_video;
end

if write_video
    vout = VideoWriter('figures/videoRRT.avi', 'Motion JPEG AVI');
    vout.Quality = 100;
    vout.FrameRate = 10;
    vout.open;
end
...
if vis
  ...
  hold on
  plot3(goal_center(1), goal_center(2), goal_center(3), 'bo')
...
% goal_center
while iter < max_iter && norm(trueQuad.x([1 5 9]) - goal_center) > goal_radius
  iter = iter + 1;
  ...
  if isempty(newStates) || sensed_new
    ...
    newStates = rrtNextState(virt_x(:,iter-1), goal_center, obsMap.padded_obs, ...
        delta_x, [], false);
    %(trueQuad.x([1 5 9]), goal_center, obsMap.padded_obs, ...
    %  delta_x, [], false);
  end
  ...
  % The safety controller is not working well
 
  if norm(virt_x(:,iter) - trueQuad.x([1 5 9])) > switch_control_margin*trackErr
      ...
      control_type = 'optimal';
       ...
       uX = LQR_Q2D(rel_x(XDims),control_gain(1));
       uY = LQR_Q2D(rel_x(YDims),control_gain(2));
       uZ = LQR_Q2D(rel_x(ZDims),control_gain(3));
...
%   fprintf('Iteration took %.2f seconds\n', toc);
  ...
  if vis && (~mod(iter,25) || iter == 2)
    ...
    if write_video
        export_fig(sprintf('figures/%d', iter), '-png')
        savefig(sprintf('figures/%d.fig', iter))
